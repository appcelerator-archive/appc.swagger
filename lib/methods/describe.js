...
exports.describe = function describe() {
	...
	return {
		...
		action: function describedAction(req, resp, next) {
			/**
			 * Handles response on POST requests.
			 * @param response
			 * @param cb
			 * @returns {*}
			 */
			function createCallback(response, cb) {
				/**
				 * Returns response in headers.location.
				 */
				return function responseCallback(err, instance) {
					if (err) {
						return cb(err);
					}
					response.set('Location', req.path + instance.getPrimaryKey());
					response.send(201);
					cb();
				};
			}
			...
			try {
				if (req.method === 'POST') {
					context.model[context.methodName](req.params, createCallback(resp.response, next));
				} else {
