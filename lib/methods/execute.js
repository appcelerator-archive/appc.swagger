var _ = require('lodash'),
	request = require('request'), // TODO Use superagent since swagger-client already does?
	Arrow = require('arrow');
...
exports.execute = function execute() {
	...
	if (connector && connector.config && connector.config.login) {
		var loginConfig = connector.config.login;
		if (loginConfig.username) {
			options.auth = {
				user: loginConfig.username,
				pass: loginConfig.password
			};
		} else if (loginConfig.apiKey) {
			var apiKey = loginConfig.apiKey;
			if (apiKey.name) {
				options.headers = {};
	...
	request(options, function (err, response, body) {
		context.handleResponse(err, response, body, function (err, result) {
