...
function sendRequest(request, callback) {
	...
	request.end(function (err, res) {
		// err contains rawResponse property which is the string we need to show in the UI
		if (err && err.rawResponse) {
			that.handleResponse(null, res, err.rawResponse, createInstanceOrCollection);
		} else {
			that.handleResponse(err, res, res.body, createInstanceOrCollection);
		}
	});

	/**
	 * Turns the provided data into model instance or collection of instances
	 *
	 * @param {Object} err
	 * @param {Object} result
	 */
	function createInstanceOrCollection(err, result) {
		/**
