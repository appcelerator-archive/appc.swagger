...
exports.create = function(APIBuilder, server) {
	...
	return Connector.extend({
		...
		logger: APIBuilder.createLogger({}, { name: 'appc.swagger' }),
		...
		connect: function connect(callback) {
			...
			if (this.config.dynamicallyLoadModels) {
				loader.loadModels(APIBuilder, this, this.config, this.logger, function(err, models) {
					if (err) {
						callback(err);
					}
					else {
						...
						server.registerModelsForConnector(self, models);
						callback();
					}
				});
			}
