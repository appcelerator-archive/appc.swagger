...
function generate(appc, opts, callback) {
	...
	async.series([
		...
		function(cb) {
			loader.loadModels(apibuilder, null, _.defaults(opts, require('../../conf/default')), null, function(err, _models) {
				models = _models;
				cb();
			});
		},
		...
		function(cb) {
			async.eachSeries(_.keys(models), function(model, done) {
				...
				buffer += "var Model = APIBuilder.Model.extend('" + model + "'," + JSON.stringify(obj, null, '\t') + ");\n\n";
