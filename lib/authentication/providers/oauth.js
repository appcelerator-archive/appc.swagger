...
var OauthAuthentication = module.exports = function (providerConfig, scopes) {
	...
	this.defaultRefreshTokenExpiryTime = providerConfig.defaultRefreshTokenExpiryTime || null;
	this.scopes = scopes;
...
OauthAuthentication.prototype.validateConfiguration = function () {
	...
	if (!_.includes(['password', 'application'], this.flow)) {
		validationErrors.push(Error('Only "password" and "application (client credentials)" flows are currntly supported.'));
	}

	if (!this.providerConfig.client_id || !this.providerConfig.client_secret) {
		validationErrors.push(Error('You must specify a client_id and client_secret for oauth providers.'));
	}

	if (!_.includes(['form', 'json'], this.requestContentType)) {
		validationErrors.push(Error('Only "form" and "json" are valid types for the requestContentType option.'));
	}

	if (!_.isNumber(this.defaultAccessTokenExpiryTime)) {
		validationErrors.push(Error('You must specify a valid expiry time (in seconds) for defaultAccessTokenExpiryTime.'));
	}

	if (this.defaultRefreshTokenExpiryTime !== null && !_.isNumber(this.defaultRefreshTokenExpiryTime)) {
		validationErrors.push(Error('You must specify a valid expiry time (in seconds) for defaultRefreshTokenExpiryTime.'));
	}
	...
	if (validationErrors.length > 0) {
		...
		throw new Error('Invalid oauth authentication provider cofiguration.' + fullErrorMessage);
	}
};
 ...
 *
 * Useful to trigger the request of new access and refresh tokens
 *
 * @return {void}
 */
